...
CREATE POLICY "Allow authenticated delete access to own profile images"
    ...
    -- -- ADD COLUMN test_score NUMERIC CHECK (test_score >= 0) DEFAULT NULL,
    -- ADD COLUMN year INTEGER CHECK (year > 0) DEFAULT NULL,
    -- ADD COLUMN city TEXT NOT NULL CHECK (char_length(city) <= 100),
    -- ADD COLUMN state TEXT NOT NULL CHECK (char_length(state) <= 100),
    -- ADD COLUMN gpa NUMERIC CHECK (gpa >= 0 AND gpa <= 4.0) DEFAULT NULL;
