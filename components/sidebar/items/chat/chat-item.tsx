...
import FilledStarIcon from "@/components/Shapes/FilledStarIcon"
...
export const ChatItem: FC<ChatItemProps> = ({ chat }) => {
  ...
  return (
    ...
    <div className="ml-3 flex-1 truncate text-xs font-semibold">
  <span className="flex items-center space-x-2">
  <span className="text-ellipsis whitespace-nowrap">{chat.name.length > CHARACTER_LIMIT
              ? `${chat.name.slice(0, CHARACTER_LIMIT)}...`
              : chat.name}</span>
    {chat.isFavorited && <FilledStarIcon />}
  </span>
</div>
