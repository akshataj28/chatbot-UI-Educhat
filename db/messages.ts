...
export const updateMessageWithFeedback = async (
  messageId: string,
  feedback: {
    thumbsUp?: boolean
    thumbsDown?: boolean
  ...
  // Prepare feedback object ensuring only non-undefined fields are included
  const updateData: { thumbs_up?: boolean; thumbs_down?: boolean; comments?: string } = {};

  if (feedback.thumbsUp !== undefined) {
    updateData.thumbs_up = feedback.thumbsUp;
  }
  
  if (feedback.thumbsDown !== undefined) {
    updateData.thumbs_down = feedback.thumbsDown;
  }
  
  if (feedback.comment !== undefined) {
    updateData.comments = feedback.comment;
  }
